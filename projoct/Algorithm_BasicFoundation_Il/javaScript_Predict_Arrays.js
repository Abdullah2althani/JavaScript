//  1. Biggie Size
//  Given an array, write a function that changes all positive numbers in the array to the string "big".
//  Example: makeItBig([-1,3,5,-5]) returns that same array, changed to [-1, "big", "big", -5].

function Biggie() {
  var arr = [-1, 3, 5, -5];
  for (let index = 0; index <= 50; index++) {
    if (arr[index] > -1) arr[index] = "big";
  }
  return arr;
}
console.log(Biggie());

//  2.Print Low, Return High
//  Create a function that takes in an array of numbers.
//  The function should print the lowest value in the array, and return the highest value in the array.

function FindLowHigh() {
  var arr = [1, 5, 10, -2];
  var max = arr[0];
  var min = arr[0];

  for (let index = 1; index < arr.length; index++) {
    if (max < arr[index]) max = arr[index];

    if (min > arr[index]) min = arr[index];
  }
  console.log(min);
  return max;
}
console.log(FindLowHigh());

//  3.Print One, Return Another
//  Build a function that takes in an array of numbers.
//  The function should print the second-to-last value in the array, and return the first odd value in the array.

function PrintOddNumber() {
  var arr = [1, 5, 10, -2, 6, 4, 8];
  var oddNumber;
  console.log(arr);
  for (let index = 1; index < arr.length; index++) {
    console.log(arr[index]);
  }

  for (let index = 0; index < arr.length; index++) {
    if (arr[index] % 2 != 0) {
      oddNumber = arr[index];
      break;
    }
  }

  return oddNumber;
}
console.log(PrintOddNumber());

//  4.Double Vision
//  Given an array (similar to saying 'takes in an array'),
//  create a function that returns a new array where each value in the original array has been doubled.
//  Calling double([1,2,3]) should return [2,4,6] without changing the original array.

function DoubleVision() {
  var arr = [1, 2, 3];
  var newarr = [];
  console.log(arr);
  for (let index = 0; index < arr.length; index++) {
    newarr[index] = arr[index] * 2;
  }
  return newarr;
}
console.log(DoubleVision());

//  5.Count Positives
//  Given an array of numbers, create a function to replace the last value with the number of positive values found in the array.
//  Example, countPositives([-1,1,1,1]) changes the original array to [-1,1,1,3] and returns it.

function CountPositives() {
  var arr = [-1, 1, 1, 1];
  var ctr = 0;
  for (let index = 0; index < arr.length; index++) {
    if (arr[index] > -1) ctr++;

    if (index == arr.length - 1) arr[index] = ctr;
  }
  return arr;
}
console.log(CountPositives());

//  6.Evens and Odds
//  Create a function that accepts an array.  Every time that array has three odd values in a row, print "That's odd!".
//  Every time the array has three evens in a row, print "Even more so!".

function EvensandOdds() {
  var arr = [1, 3, 5, 7, 20, 11, 21, 23];
  var ctrOdd = 0;
  var ctrEven = 0;
  for (let index = 0; index < arr.length; index++) {
    if (arr[index] % 2 == 0) {
      ctrEven++;
      if (ctrEven == 3) {
        console.log("Even more so!");
        ctrEven = 0;
      }
    } else if (arr[index] % 2 != 0) {
      ctrOdd++;
      if (ctrOdd == 3) {
        console.log("That's odd!");
        ctrOdd = 0;
      }
    }
  }
  
  return arr;
}
console.log(EvensandOdds());

//  7.Increment the Seconds
//  Given an array of numbers arr, add 1 to every other element,
//  specifically those whose index is odd (arr[1], arr[3], arr[5], etc).  Afterward, console.log each array value and return arr.

function ArrayOdd() {
  var arr = [1, 3, 5, 7];
  console.log(arr);

  for (let index = 1; index < arr.length; index++) {
    if (index % 2 != 0) 
    arr[index] = arr[index] + 1 
  }

  return arr;
}
console.log(ArrayOdd());

//  8.Previous Lengths
//  You are passed an array (similar to saying 'takes in an array' or 'given an array') containing strings.
//  Working within that same array, replace each string with a number - the length of the string at the previous array index - and return the array.
//  For example, previousLengths(["hello", "dojo", "awesome"]) should return ["hello", 5, 4]. Hint: Can for loops only go forward?

// TODO: needs help (not understood )

/* function Lengths() {
   var arr = ["hello", "dojo", "awesome"]
   console.log(arr);
   var itm = ""
   for (let index = 0; index < arr[index]; index++) {
     itm = arr[index]
     arr[index] = itm.length    
   }

   return itm.length;
 }
 console.log(Lengths());
*/

// 9.Add Seven
// Build a function that accepts an array. Return a new array with all the values of the original, but add 7 to each. Do not alter the original array.
// Example, addSeven([1,2,3]) should return [8,9,10] in a new array.

function Squares() {
  var arr = [1, 5, 10, -2];
  console.log(arr);
  for (let index = 0; index < arr.length; index++) {
    arr[index] = arr[index] + 7
  }

  return arr;
}
console.log(Squares());

// 10.Reverse Array
// Given an array, write a function that reverses its values, in-place.
// Example: reverse([3,1,6,4,2]) returns the same array, but now contains values reversed like so... [2,4,6,1,3].
// Do this without creating an empty temporary array.  (Hint: you'll need to swap values).

// TODO: needs help (not understood )

// 11. Outlook: Negative
// Given an array, create and return a new one containing all the values of the original array,
// but make them all negative (not simply multiplied by -1). Given [1,-3,5], return [-1,-3,-5].

function Negatives() {
    var arr = [1,-3,5]
    for (let index = 0; index < arr.length; index++) {
      if(arr[index]<-1)
      continue
      else
      arr[index] = arr[index] * -1
    }
  
    return arr;
  }
  console.log(Negatives());

// 12. Always Hungry
// Create a function that accepts an array, and prints "yummy" each time one of the values is equal to "food".
// If no array values are "food", then print "I'm hungry" once.

function SwapValue() {
  var arr = [1, 5, "food", -2];
  var ctr = 0
  for (let index = 0; index < arr.length; index++) {
    if (arr[index] == "food") {
    console.log("yummy");
    ctr++      
    }
  }
  if(ctr == 0){
    console.log("I'm hungry");
  }

  return arr;
}
console.log(SwapValue());

// 13. Swap Toward the Center
// Given an array, swap the first and last values, third and third-to-last values, etc.
// Example: swapTowardCenter([true,42,"Ada",2,"pizza"]) turns the array into ["pizza", 42, "Ada", 2, true].
// swapTowardCenter([1,2,3,4,5,6]) turns the array into [6,2,4,3,5,1].  No need to return the array this time.

// TODO: needs help (not understood )

// 14.Scale the Array
// Given an array arr and a number num,
// multiply all values in the array arr by the number num, and return the changed array arr.
// For example, scaleArray([1,2,3], 3) should return [3,6,9].

function scaleArray() {
    var arr = [1,2,3]
    var scal = 3;
    for (let index = 0; index < arr.length; index++) 
      arr[index] = arr[index] * scal
  
    return arr;
  }
  console.log(scaleArray());
